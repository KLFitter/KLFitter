...
int KLFitter::Permutations::SetPermutation(int index)
{
	...
	// check index 
	if (index < 0 || index >= this -> NPermutations())
		{
...
int KLFitter::Permutations::CreatePermutations()
{
	...
	fTablePartons = new std::vector < std::vector<int> * >(0); 
 	this -> CreateSubTable(npartons, fTablePartons); 

	fTableElectrons = new std::vector < std::vector<int> * >(0); 
	this -> CreateSubTable(nelectrons, fTableElectrons); 

	fTableMuons = new std::vector < std::vector<int> * >(0); 
	this -> CreateSubTable(nmuons, fTableMuons); 

	fTablePhotons = new std::vector < std::vector<int> * >(0); 
	this -> CreateSubTable(nphotons, fTablePhotons); 
							...
							for (int i = 0; i < npartons; ++i)
								{
									...
									// get parton 
									TLorentzVector * vect = (*fParticles) -> Parton(index); 

									// add parton 
									particles -> AddParticle(vect, KLFitter::Particles::kParton); 
							...
							for (int i = 0; i < nelectrons; ++i)
								{
									...
									// get electron 
									TLorentzVector * vect = (*fParticles) -> Electron(index); 

									// add electron 
									particles -> AddParticle(vect, KLFitter::Particles::kElectron); 
							...
							for (int i = 0; i < nmuons; ++i)
								{
									...
									// get muon 
									TLorentzVector * vect = (*fParticles) -> Muon(index); 

									// add muon 
									particles -> AddParticle(vect, KLFitter::Particles::kMuon); 
							...
							for (int i = 0; i < nphotons; ++i)
								{
									...
									// get photon 
									TLorentzVector * vect = (*fParticles) -> Photon(index); 

									// add photon 
									particles -> AddParticle(vect, KLFitter::Particles::kPhoton); 
					...
					for (unsigned int i = 0; i < indexVector.size()-1; i++)
						{
							...
							err *= this -> InvariantParticlePermutations(ptype, newIndexVector);
						}
