...
int main(int argc, char **argv) 
{
  ...
  if (BeamEnergy==KLFitter::DetectorBase::k7TeV)
    ...
    myDetector = new KLFitter::DetectorAtlas_10TeV("../../transferfunctions/10TeV/ttbar");
		...
		std::cout<<"Error: Detector could not be created, please check the transferfunction flags"<<std::endl;
		return 1;
	}
  if (!myFitter -> SetDetector(myDetector))
    return 0; 

  // create likelihood for ttbarH->l+jets channel 
  ...
  myInterfaceRoot -> WriteSignalMCTruth(FlagWriteSignalMCTruth, KLFitter::InterfaceRoot::kAcer);
  ...
  mySelectionTool -> RequireNJetsPt(25.0, 6, 0); 
  ...
  if (maxEv>nevents) {
	...
	else if (minEv ==  0 && maxEv == -1) {
		maxEv = nevents;
		std::cout << "run over all " << nevents << " events." << std::endl;
	}
  if (minEv>=maxEv) {
  ...
  for (int ievent = minEv; ievent < maxEv; ++ievent)
    {
      ...
      if (myMatchingTool)
        {
          ...
          std::vector<int> v5 = myMatchingTool->ListMatchedTruth(5, KLFitter::Particles::kParton); // light quark 2
					...
					// debugKK
					std::cout << GetTrueCombinationString(v0, v1, v2, v3, v4, v5) << std::endl;
					//					int trueCombi = GetCombinationNo(GetTrueCombinationString(v0, v1, v2, v3, v4, v5));
      ...
      if (firstevent) {
        printf("----------------------------------------------------------------------------------------------\n");
			...
			// debugKK: no fitting, just counting
			/*
